...
class DBTest {
    ...
    fun testUpsertAndRetrieve() = runBlocking {
        // Test upsertRecept
        val recept = Recept(naziv = "Recept 1", uputstvo = "Uputstvo za recept 1")
        val receptId = dao.upsertRecept(recept)
        assert(receptId > 0)

        // Test upsertSastojak
        val sastojak = Sastojak(naziv = "Sastojak 1", kolicina = 10, jedinicaMere = "kom", tortaId = 1)
        val sastojakId = dao.upsertSastojak(sastojak)
        assert(sastojakId > 0)

        // Test upsertTorta
        val torta = Torta(naziv = "Torta 1", opis = "Opis za tortu 1", tezina = 4000)
    ...
    fun testListaTortiSaKomSastojcima() = runBlocking {
        val torta = Torta(naziv = "Torta 1", opis = "Opis za tortu 1", tezina = 2000)
        val tortaId = dao.upsertTorta(torta)

        val sastojak = Sastojak(naziv = "Sastojak 1", kolicina = 10, jedinicaMere = "kom", tortaId = tortaId.toInt())
    ...
    fun testListaTortiTezeOdDvaKilograma() = runBlocking {
        ...
        val sastojak = Sastojak(naziv = "Sastojak 1", kolicina = 3000, jedinicaMere = "g", tortaId = tortaId.toInt())
        dao.upsertSastojak(sastojak)
